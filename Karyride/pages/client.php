...
| - Address autocomplete (Uber-style dropdown)
...
.input-wrap input{
    ...
    padding:0 90px 0 40px;font-size:13px;outline:0;
...
.clear-input{
    position:absolute;right:46px;top:7px;width:36px;height:36px;border:0;
    border-radius:9px;background:transparent;color:var(--muted);display:none;z-index:3;
    align-items:center;justify-content:center
}
.clear-input.show{display:flex}
.map-pick{
    position:absolute;right:6px;top:7px;width:36px;height:36px;border:0;
    border-radius:9px;background:transparent;color:var(--muted);z-index:3;
    display:flex;align-items:center;justify-content:center
}
.map-pick:hover,.map-pick.active{background:var(--accent-soft);color:var(--accent)}
.map-wrap.pick-mode #map{cursor:crosshair}
.map-pick.active{box-shadow:inset 0 0 0 1px var(--accent)}
.pick-hint{
...
.suggestions{
    position:absolute;left:0;right:0;top:56px;z-index:1200;
...
.suggestion:hover,.suggestion.active-option{background:var(--surface-2)}
.suggestion i{color:var(--accent);margin-top:2px;flex:none}
...
.suggestion-empty,.suggestion-loading{
    padding:14px 13px;font-size:12px;color:var(--muted);display:flex;align-items:center;gap:9px
}
.suggestion-loading i{animation:spin .8s linear infinite}
    ...
    <aside class="sidebar">
        ...
        <div class="field">
            ...
            <div class="input-wrap">
                ...
                <input id="pickupInput" type="text" autocomplete="off" placeholder="Enter pickup address">
        ...
        <div class="field">
            ...
            <div class="input-wrap">
                ...
                <input id="destinationInput" type="text" autocomplete="off" placeholder="Where should we take you?">
...
/*
|--------------------------------------------------------------------------
| KaryRide Client JavaScript — Uber-Style
|--------------------------------------------------------------------------
| Features:
...
const storedTheme = localStorage.getItem('karyride_theme');
...
let searchTimers = { pickup: null, destination: null };
let searchControllers = { pickup: null, destination: null };
let searchTokens = { pickup: 0, destination: 0 };
let nearbyRefreshInterval = null;
let isNearbyLoading = false;
...
function driverIcon(distance) {
    const color = distance < 1 ? '#16a34a' : distance < 3 ? '#f59e0b' : '#dc2626';
    return L.divIcon({
        className: '',
        html: `
            <div class="driver-marker" style="border-color:${color};color:${color};">
                <i class="bi bi-car-front-fill"></i>
...
function setMapStatus(title, hint = '') {
    document.getElementById('mapStatus').textContent = title;
...
function showMessage(text, error = false, success = false) {
    const el = document.getElementById('bookMessage');
    el.textContent = text;
    el.className = 'message' + (error ? ' error' : '') + (success ? ' success' : '');
}

function updateClearButtons() {
    document.getElementById('clearPickup').classList.toggle('show', !!document.getElementById('pickupInput').value);
    document.getElementById('clearDestination').classList.toggle('show', !!document.getElementById('destinationInput').value);
}

/* -----------------------------
   Set Point (Pickup/Destination)
----------------------------- */
function setPoint(type, result, updateInput = true, moveMap = true) {
    ...
    if (type === 'pickup') {
        ...
        if (updateInput) document.getElementById('pickupInput').value = point.address || `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
    } else {
        ...
        if (updateInput) document.getElementById('destinationInput').value = point.address || `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
    }

    // Always fully close out any pending search state for this field so the
    // dropdown and input stay in sync with what was actually picked.
    cancelPendingSearch(type);
    hideSuggestions(type === 'pickup' ? pickupSuggestions : destinationSuggestions);
    updateClearButtons();
...
/* -----------------------------
   Address Autocomplete (Uber-Style)
----------------------------- */
...
function cancelPendingSearch(type) {
    ...
    // Bump the token so any in-flight response for this field is ignored.
    searchTokens[type] += 1;
}

function showSuggestionLoading(box) {
    box.innerHTML = `<div class="suggestion-loading"><i class="bi bi-arrow-repeat"></i> Searching addresses...</div>`;
    box.classList.add('show');
}

function showSuggestionEmpty(box) {
    box.innerHTML = `<div class="suggestion-empty"><i class="bi bi-search"></i> No matching addresses found</div>`;
    box.classList.add('show');
}

function renderSuggestions(box, results, type) {
    box.innerHTML = '';
    if (!results || results.length === 0) { showSuggestionEmpty(box); return; }

    // Guarantee exactly SUGGESTION_LIMIT (or fewer) results, always in the
    // same order the API returned them (best match first).
    results.slice(0, SUGGESTION_LIMIT).forEach(result => {
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'suggestion';

        const nameParts = String(result.display_name || '').split(',');
        const title = nameParts.shift()?.trim() || 'Location';
        const detail = nameParts.join(',').trim();
        ...
        // Use mousedown so the pick registers before the input's blur event
        // has a chance to hide the dropdown first — this makes selection
...
async function searchAddresses(type, value) {
    const box = type === 'pickup' ? pickupSuggestions : destinationSuggestions;
    const input = type === 'pickup' ? pickupInput : destinationInput;

    const query = value.trim();
    if (query.length < MIN_QUERY_LENGTH) { hideSuggestions(box); return; }

    // Cancel any in-flight request for this field before starting a new one
    // so slow, stale responses can never overwrite fresher results.
    if (searchControllers[type]) searchControllers[type].abort();
    const controller = new AbortController();
    searchControllers[type] = controller;
    const token = ++searchTokens[type];

    showSuggestionLoading(box);
    ...
    try {
        ...
        // Only render if this is still the latest request for this field
        // and the input text hasn't changed since the request was sent.
        if (token === searchTokens[type] && input.value.trim() === query) {
            renderSuggestions(box, results, type);
        }
    } catch (error) {
        if (error.name === 'AbortError') return;
        if (token === searchTokens[type]) hideSuggestions(box);
    }
}
...
function scheduleSearch(type, value) {
    ...
    searchTimers[type] = setTimeout(() => searchAddresses(type, value), SEARCH_DEBOUNCE_MS);
}
...
pickupInput.addEventListener('focus', function() {
    if (this.value.trim().length >= MIN_QUERY_LENGTH && !pickupPoint) {
        scheduleSearch('pickup', this.value);
    }
});

destinationInput.addEventListener('focus', function() {
    if (this.value.trim().length >= MIN_QUERY_LENGTH && !destinationPoint) {
        scheduleSearch('destination', this.value);
    }
});

// Close suggestions on blur (short delay so a mousedown pick still fires)
pickupInput.addEventListener('blur', function() {
    setTimeout(() => hideSuggestions(pickupSuggestions), 150);
});
...
const pickPickupOnMap = document.getElementById('pickPickupOnMap');
const pickDestinationOnMap = document.getElementById('pickDestinationOnMap');
...
function setMapPickMode(type) {
    ...
    if (type) {
        // Entering map-pick mode should always close any open dropdown so
        // the two input states never fight for the user's attention.
        hideSuggestions(pickupSuggestions);
        hideSuggestions(destinationSuggestions);
    ...
    navigator.geolocation.getCurrentPosition(async function(position) {
        ...
        try {
            ...
            if (data?.display_name) {
                setPoint('pickup', { lat, lng, display_name: data.display_name });
                setMapStatus('Pickup detected', 'Your current location was used as pickup.');
                updateRoute();
            } else {
                setPoint('pickup', { lat, lng, display_name: `${lat.toFixed(6)}, ${lng.toFixed(6)}` });
                setMapStatus('Pickup detected', 'Could not resolve the address name.');
            }
        } catch {
            setPoint('pickup', { lat, lng, display_name: `${lat.toFixed(6)}, ${lng.toFixed(6)}` });
...
async function updateRoute() {
    ...
    try {
        const url = `${ROUTER_URL}/${pickupPoint.lng},${pickupPoint.lat};${destinationPoint.lng},${destinationPoint.lat}`;
...
function invalidateTrip(clearMessage = true) {
    routeDistanceKm = 0;
    routeDurationMin = 0;
...
async function reverseCurrentLocation(lat, lng) {
    try {
        ...
        if (data.display_name) {
            setPoint('pickup', {lat: lat, lng: lng, display_name: data.display_name});
